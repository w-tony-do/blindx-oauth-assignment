...
describe('PrescriptionForm', () => {
  ...
  it('shows alert when submitting without selecting a medication', async () => {
    ...
    const form = screen.getByRole('button', { name: 'Create Prescription' }).closest('form');
  ...
  it('shows delivery address fields when "Same as patient address" is unchecked', async () => {
    ...
    expect(screen.getByLabelText('Address Line 1 *', { selector: '#del_address_ln1' })).toBeInTheDocument();
  });
  ...
  it('uses separate delivery address when checkbox is unchecked', async () => {
    ...
    const deliveryAddressInput = screen.getByLabelText('Address Line 1 *', { selector: '#del_address_ln1' });
    ...
    const submittedData: PrescriptionFormData = mockOnSubmit.mock.calls[0][0];
    expect(submittedData.delivery_address.address_ln1).toBe('456 Different Street');
  });
  ...
  it('disables submit button when loading', () => {
    ...
    const submitButton = screen.getByRole('button', { name: /Creating Prescription/i });
  ...
  it('displays different button text when loading', () => {
    ...
    expect(screen.getByRole('button', { name: 'Creating Prescription...' })).toBeInTheDocument();
  });
  ...
  it('updates address fields', async () => {
    ...
    const address1Input = screen.getByLabelText('Address Line 1 *', { selector: '#address_ln1' });
    const address2Input = screen.getByLabelText('Address Line 2', { selector: '#address_ln2' });
  ...
  it('updates delivery address fields when using separate address', async () => {
    ...
    const delAddress1 = screen.getByLabelText('Address Line 1 *', { selector: '#del_address_ln1' });
    const delAddress2 = screen.getByLabelText('Address Line 2', { selector: '#del_address_ln2' });
    const delCity = screen.getByLabelText('City *', { selector: '#del_city' });
    const delPostCode = screen.getByLabelText('Post Code *', { selector: '#del_post_code' });
    const delCountry = screen.getByLabelText('Country *', { selector: '#del_country' });
