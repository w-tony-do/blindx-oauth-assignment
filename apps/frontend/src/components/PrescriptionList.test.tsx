...
describe('PrescriptionList', () => {
  ...
  it('renders empty state when no prescriptions exist', () => {
    ...
    expect(
      screen.getByText('No prescriptions yet. Create your first prescription above!')
    ).toBeInTheDocument();
  });
  ...
  it('displays "Pending" when prescription ID is not available', () => {
    ...
    render(
      <PrescriptionList prescriptions={prescriptionsWithoutId} isLoading={false} />
    );
  ...
  it('renders prescription cards with correct structure', () => {
    ...
    const cards = screen.getAllByText(/Prescription ID:/i).map(el => el.closest('.prescription-card'));
  ...
  it('handles empty medicines array gracefully', () => {
    ...
    render(
      <PrescriptionList prescriptions={prescriptionWithNoMeds} isLoading={false} />
    );
